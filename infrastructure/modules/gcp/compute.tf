...
resource "google_compute_instance" "foundry" {
  ...
  # Cloud Init user data and OS Login — merged into a single metadata block
  # as the Google provider replaces duplicate metadata blocks rather than merging them.
  # user_data_raw is used directly since GCE metadata expects plain cloud-init text.
  metadata = {
